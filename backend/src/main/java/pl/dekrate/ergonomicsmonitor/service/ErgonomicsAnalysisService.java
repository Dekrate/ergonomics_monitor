...
/**
 * Service responsible for analyzing user activity data using Artificial Intelligence. It
 * periodically fetches aggregated data from the database and consults a local LLM (Bielik via
 * Ollama) to assess ergonomic risks and fatigue levels.
 *
 * @author dekrate
 * @version 1.0
 */
...
public class ErgonomicsAnalysisService {
     ...
     */
    public ErgonomicsAnalysisService(
            ActivityRepository repository, ChatClient.Builder chatClientBuilder) {
        this.repository = repository;
        this.chatClient = chatClientBuilder.build();
    }

    /**
     * Scheduled task that runs every 5 minutes to perform AI-driven analysis. It uses a reactive
     * pipeline to gather data and non-blocking AI calls. The processing is offloaded to Virtual
     * Threads by the underlying framework.
     */
                            ...
                            String dataSummary =
                                    events.stream()
                                            .map(
                                                    e ->
                                                            String.format(
                                                                    "Time: %s, Type: %s, Intensity: %.1f, Metadata: %s",
                                                                    e.getTimestamp(),
                                                                    e.getType(),
                                                                    e.getIntensity(),
                                                                    e.getMetadata()))
