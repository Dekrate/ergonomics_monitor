...
public final class AIBreakRecommendationStrategy implements IntensityAnalysisStrategy {
    ...
    private static final String ANALYSIS_PROMPT_PL =
            """
        ...
        """;
    private static final String ANALYSIS_PROMPT_EN =
            """
        You are an ergonomics expert. Analyze the following user activity data:

        Activity statistics (last {timeWindow} minutes):
        - Total events: {totalEvents}
        - Average intensity: {avgIntensity}
        - Max intensity: {maxIntensity}
        - Work duration: {workDurationMinutes} minutes

        Activity patterns:
        {activityPatterns}

        Based on this data, assess ergonomic risk and recommend a break if needed.
    ...
    public AIBreakRecommendationStrategy(
            ChatClient chatClient, AiLanguagePreferenceService languagePreferenceService) {
    ...
    private Mono<String> queryAI(Map<String, Object> analysisData) {
        return Mono.fromCallable(
                        () -> {
                            ...
                            log.info(
                                    ">>> [AI START] Sending data to Ollama (Bielik). language={}",
                                    language);
                            log.debug("Analysis Data: {}", analysisData);

                            PromptTemplate promptTemplate =
                                    new PromptTemplate(getPromptTemplate(language));
                            ...
                            log.info(
                                    ...
                                    duration);
                            return withLanguagePrefix(content, language);
                        })
        ...
        return Mono.fromCallable(
                () -> {
                    try {
                        ...
                        if (response.startsWith("LANG=PL\n")) {
                            language = AiLanguage.PL;
                            response = response.substring("LANG=PL\n".length());
                        } else if (response.startsWith("LANG=EN\n")) {
                            response = response.substring("LANG=EN\n".length());
                        }

                        String cleaned =
                                response.trim().replace("```json", "").replace("```", "").trim();
                        ...
                        return BreakRecommendation.builder()
                                ...
                                .reason(formatAiReason(language, reason))
                                .metrics(
    ...
    private String getPromptTemplate(AiLanguage language) {
        return language == AiLanguage.PL ? ANALYSIS_PROMPT_PL : ANALYSIS_PROMPT_EN;
    }

    private String withLanguagePrefix(String content, AiLanguage language) {
        return "LANG=" + language + "\n" + content;
    }

    private String formatAiReason(AiLanguage language, String reason) {
        return switch (language) {
            case PL -> "Analiza AI: " + reason;
            case EN -> "AI Analysis: " + reason;
        };
    }
