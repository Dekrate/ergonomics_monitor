...
/**
 * Comprehensive unit tests for ActivityIntensityMetrics value object.
 * Testing all business logic, edge cases, and invariants.
 *
 * @author dekrate
 * @version 1.0
 */
...
class ActivityIntensityMetricsTest {
	...
	class BuilderTests {
		...
		void shouldBuildValidMetrics() {
			...
			// when
			ActivityIntensityMetrics metrics = ActivityIntensityMetrics.builder()
		...
		void shouldThrowNPEWhenTimeWindowIsNull() {
			// given
			ActivityIntensityMetrics.Builder builder = ActivityIntensityMetrics.builder()
					.totalEvents(100)
					.timeWindow(null);
	...
	class EventsPerMinuteTests {
		...
		void shouldCalculateEventsPerMinuteCorrectly() {
			...
			// then
			assertEquals(15.0, eventsPerMinute, 0.01,
					"150 events / 10 minutes = 15 events/min");
		}
		...
		void shouldHandleFractionalMinutes() {
			...
			// then
			assertEquals(100.0, eventsPerMinute, 0.01,
					"150 events / 1.5 minutes = 100 events/min");
		}
	...
	class EqualityTests {
		...
		void shouldNotBeEqualWhenTotalEventsDiffer() {
			// given
			ActivityIntensityMetrics metrics1 = ActivityIntensityMetrics.builder()
					.totalEvents(100)
					.timeWindow(Duration.ofMinutes(10))
					.build();

			ActivityIntensityMetrics metrics2 = ActivityIntensityMetrics.builder()
					.totalEvents(200)
					.timeWindow(Duration.ofMinutes(10))
					.build();
		...
		class ToStringTests {
			...
			void shouldIncludeAllInformation() {
				...
				// then
				assertAll("toString contains all fields",
