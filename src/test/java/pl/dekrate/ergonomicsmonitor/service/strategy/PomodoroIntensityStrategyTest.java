...
/**
 * Comprehensive tests for PomodoroIntensityStrategy.
 * Tests all business scenarios including edge cases.
 *
 * @author dekrate
 * @version 1.0
 */
...
class PomodoroIntensityStrategyTest {
    ...
    @Test
    @DisplayName("should return MEDIUM urgency recommendation when intensity is moderate (100-200 events/min)")
        ...
        StepVerifier.create(result)
                .assertNext(recommendation -> {
                    assertAll(
                            ...
                            () -> assertEquals(5, recommendation.getSuggestedBreakDuration().toMinutes()),
                            () -> assertTrue(recommendation.getReason().contains("Intensywna praca")),
                            () -> assertEquals(3000, recommendation.getMetrics().getTotalEvents()),
                            () -> assertEquals(1800, recommendation.getMetrics().getKeyboardEvents()),
                            () -> assertEquals(1200, recommendation.getMetrics().getMouseEvents())
                    );
                })
                .expectComplete()
                .verify();
    ...
    @Test
    @DisplayName("should return CRITICAL urgency recommendation when intensity is high (>200 events/min)")
        ...
        StepVerifier.create(result)
                .assertNext(recommendation -> {
                    assertAll(
                            ...
                            () -> assertEquals(10, recommendation.getSuggestedBreakDuration().toMinutes()),
                            () -> assertTrue(recommendation.getReason().contains("bardzo intensywną pracę")),
                            () -> assertTrue(recommendation.getReason().contains("RSI")),
                            () -> assertEquals(6250, recommendation.getMetrics().getTotalEvents())
                    );
                })
                .expectComplete()
                .verify();
