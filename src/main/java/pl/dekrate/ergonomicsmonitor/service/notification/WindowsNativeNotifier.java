...
/**
 * Windows-native notification adapter using JNA to display MessageBox dialogs.
 * This is a concrete adapter implementing the BreakNotifier port.
 * <p>
 * Implementation notes:
 * - MessageBox calls are BLOCKING by design (Windows API limitation)
 * - We execute on boundedElastic scheduler to avoid blocking reactive pipeline
 * - Uses MB_TOPMOST flag to ensure notification is visible
 * - Provides better UX than toast notifications (user must acknowledge)
 * <p>
 * Conditional bean - only loaded on Windows platforms and when enabled in config.
 *
 * @author dekrate
 * @version 1.0
 */
@Component
@ConditionalOnProperty(name = "ergonomics.notifications.windows.enabled", havingValue = "true", matchIfMissing = true)
public class WindowsNativeNotifier implements BreakNotifier {
    ...
    private static final String NOTIFICATION_TITLE = "Ergonomics Monitor - Przypomnienie o przerwie";
    ...
    public interface User32Extended extends StdCallLibrary {
        ...
        /**
         * MessageBox with all parameters for maximum control.
         * Method name matches Windows API convention.
         */
    ...
    public Mono<Void> sendNotification(BreakRecommendation recommendation) {
        return Mono.fromRunnable(() -> {
            try {
                ...
                log.info("Displaying Windows MessageBox: urgency={}", recommendation.getUrgency());

                // MessageBox is BLOCKING - but we're on boundedElastic scheduler, so it's safe
                ...
                int result = User32Extended.INSTANCE.messageBoxW(
                ...
                log.error(
                    "Failed to display Windows notification: JNA library not available", e);
            } catch (IllegalArgumentException e) {
                // Invalid arguments to JNA call
                log.error(
                    "Failed to display Windows notification: invalid arguments", e);
    ...
    /**
     * Formats the recommendation into a user-friendly message.
     * Uses Windows-style line breaks for better readability.
     */
    private String formatMessage(BreakRecommendation recommendation) {
        ...
        return """
                ...
                Statystyki aktywnosci:\r
                  ...
                  - Zdarzenia myszy: %,d\r
                  - Intensywnosc: %.1f zdarzen/min""".formatted(
