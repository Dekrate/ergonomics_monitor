 ...
 *
 * <p>Responsibilities (following SRP): - Schedule periodic activity analysis - Coordinate between
 * strategy and notification layers - Maintain state to prevent notification spam
 *
 * <p>Design principles applied: - Dependency Inversion: Depends on abstractions
 * (IntensityAnalysisStrategy, BreakNotifier) - Single Responsibility: Only orchestrates, delegates
 * actual work - Open/Closed: New strategies/notifiers can be added without modifying this class
 *
 * @author dekrate
 * @version 1.0
 */
...
public class BreakNotificationService {
    ...
    /**
     * Constructor injection ensures all dependencies are provided and immutable. Spring
     * automatically injects all beans implementing the interface types.
     */
    public BreakNotificationService(
        ...
        log.info(
                "BreakNotificationService initialized with {} strategies and {} notifiers",
                analysisStrategies.size(),
                notifiers.size());
    ...
    /**
     * Scheduled task running every minute to check if user needs a break. Uses cron expression for
     * precise scheduling.
     */
    ...
    /**
     * Fetches recent activity events from the repository. Limits to last 50 events to analyze
     * approximately 25 minutes of activity.
     */
    ...
    /**
     * Runs all registered analysis strategies and returns the first recommendation found. Uses Flux
     * to process strategies in order until one produces a result.
     */
    private Mono<BreakRecommendation> analyzeWithAllStrategies(List<ActivityEvent> events) {
        ...
        return Flux.fromIterable(analysisStrategies)
                ...
                .doOnNext(
                        rec ->
                                log.info(
                                        "Break recommendation generated: urgency={}, reason={}",
                                        rec.getUrgency(),
                                        rec.getReason()));
    }
    ...
    /** For testing: allows resetting notification throttle. Package-private for test access. */
    @SuppressWarnings("java:S1144") // Used by tests
    void resetNotificationThrottle() {
        lastNotificationTime = Instant.EPOCH;
    }

    /** For testing: provides last notification time. Package-private for test access. */
    @SuppressWarnings("java:S1144") // Used by tests
